export const GMAIL_TOKEN_URI = 'https://oauth2.googleapis.com/token';

export const GMAIL_API_BASE_URL = 'https://www.googleapis.com/gmail/v1/users';

export const GMAIL_PEOPLE_API_BASE_URL = 'https://people.googleapis.com/v1';

export const GMAIL_APIs = {
    PROFILE: '/me/profile',
    MESSAGES: '/me/messages',
    BATCH_DELETE: '/me/messages/batchDelete',
    BATCH_MODIFY: '/me/messages/batchModify',
    HISTORY: '/me/history',
    LABELS: '/me/labels',
};

export const GMAIL_PEOPLE_APIs = {
    OTHER_CONTACTS: '/otherContacts:search',
};

export const GMAIL_USER_INFO = 'https://www.googleapis.com/oauth2/v3/userinfo';
