...
> **Type:** Master Plan · **Scope:** Full product feature roadmap · **Baseline:** v2.1.0 (post Background Sync)
...
├── email-experience-completion-plan.md  ← Phase-specific implementation plan
├── [future-phase]-plan.md               ← Created before each new phase begins
└── ...
```
...
- **Backend:** Add `getEmailsByThread(threadId)` in `EmailRepository` and `EmailService`, new endpoint `GET /emails/thread/:threadId`
- **Frontend:** `ThreadView` component replacing single-email view, collapsible message bubbles per thread, reply-in-thread support
- **Effort:** Medium (data exists, needs grouping + UI)

#### 1.2 Attachments (Preview + Download)
- **Why:** A huge portion of emails have attachments. Without this, users must switch to their native email client.
- **Shared Types:** Add `attachments` field to `EmailAttributes` in `@mailsense/types`
- **Backend:** Extend `EmailSchema` with `attachments: [{ filename, mimeType, size, attachmentId }]`, parse attachments during sync (Gmail `payload.parts`, Outlook `attachments` endpoint), add `GET /emails/attachment/:emailId/:attachmentId` download proxy
- **Frontend:** Attachment badge in email list, attachment list with icons in email detail, inline image preview, download action
- **Effort:** Medium-High (provider API work + new UI components)
